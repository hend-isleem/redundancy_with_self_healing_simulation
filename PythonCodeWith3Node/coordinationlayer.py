...
class MAPEKMQTTManager:
    ...
    def analyze_and_plan(self):
        ...
        # Check heartbeat timeouts for each node
        ...
        # Check data consistency once per cycle (not per node)
        self.check_data_consistency(current_time)

    def check_data_consistency(self, current_time: int):
        """Check data consistency based on available nodes"""
        active_nodes = [n for n in self.nodes.values() if n.active and not n.quarantined and n.distance > 0]
        
        if not active_nodes:
            self.logger.warning(f"{bcolors.WARNING}[ANALYZE] No active nodes with valid data{bcolors.ENDC}")
            return
        
        # Only perform Byzantine fault detection with 3+ nodes
        if len(active_nodes) >= 3:
            ...
            if outliers:
                for outlier_node in outliers:
                    ...
                    self.logger.warning(f"{bcolors.WARNING}[ANALYZE] Byzantine fault detected in Node {outlier_node.node_id}{bcolors.ENDC}")
    ...
    def handle_single_node(self, active_nodes: List[NodeStatus]) -> Optional[float]:
        """Handle case with only 1 active node - return its value directly"""
        if len(active_nodes) == 1:
            self.logger.info(f"{bcolors.OKBLUE}[SINGLE_NODE] Using Node {active_nodes[0].node_id} value: {active_nodes[0].distance:.2f} cm{bcolors.ENDC}")
            return active_nodes[0].distance
        return None
    
    def handle_two_nodes(self, active_nodes: List[NodeStatus]) -> Optional[float]:
        """Handle case with 2 active nodes - return average"""
        if len(active_nodes) == 2:
            distances = [node.distance for node in active_nodes]
            average = sum(distances) / len(distances)
            self.logger.info(f"{bcolors.OKCYAN}[TWO_NODES] Average of Node {active_nodes[0].node_id} ({distances[0]:.2f}) and Node {active_nodes[1].node_id} ({distances[1]:.2f}): {average:.2f} cm{bcolors.ENDC}")
            return average
        return None
    
    def handle_three_nodes(self, active_nodes: List[NodeStatus]) -> Optional[float]:
        """Handle case with 3 active nodes - use majority voting (median)"""
        if len(active_nodes) >= 3:
            distances = [node.distance for node in active_nodes]
            distances.sort()
            median = distances[len(distances)//2]
            self.logger.info(f"{bcolors.OKORANGE}[THREE_NODES] Majority voting result (median): {median:.2f} cm{bcolors.ENDC}")
            return median
        return None

    def calculate_consensus(self) -> Optional[float]:
        """Calculate consensus based on number of active nodes"""
        ...
        # Route to appropriate handler based on node count
        if len(active_nodes) == 1:
            return self.handle_single_node(active_nodes)
        elif len(active_nodes) == 2:
            return self.handle_two_nodes(active_nodes)
        else:
            return self.handle_three_nodes(active_nodes)
    ...
    def publish_consensus(self, consensus: Optional[float] = None):
        ...
        if consensus is None:
            ...
            # self.logger.error(f"{bcolors.FAIL}[EXECUTE] Cannot publish consensus: No valid consensus value{bcolors.ENDC}")
            # return
