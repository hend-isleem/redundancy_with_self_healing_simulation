...
void callback(char* topic, byte* payload, unsigned int length) {
    ...
    Serial.printf("[WORKER %d] Command received: %s\n", NODE_ID, msg.c_str());

    if (msg.startsWith("REBOOT:")) {
        int target = msg.substring(7).toInt();
        if (target == NODE_ID) {
            Serial.printf("[WORKER %d] Rebooting now...\n", NODE_ID);
            delay(500);
            ESP.restart();
        }
    }
...
void setup() {
  ...
  char msg[50];
  sprintf(msg, "RECOVERED:%d", NODE_ID);
  client.publish("esp32/system/recovery", msg);
}
